...
const RedeemGCP = () => {
  ...
  return (
    <Container className="py-5">
      <h2 className="text-center text-success fw-bold mb-4">Redeem Your EcoPerks</h2>
      <Row>
          ...
          <Col md={4} className="mb-4" key={index}>
            <Card className="shadow-sm">
              ...
              <Card.Body>
                ...
                <Card.Text>
                  <Badge bg="info" className="me-2">{reward.type}</Badge>
                  {reward.option && <Badge bg="secondary">{reward.option}</Badge>}
                </Card.Text>
                <Card.Text className="text-success fw-bold">💰 {reward.price}</Card.Text>
                {reward.offer && <p className="text-danger fw-bold">🔥 {reward.offer}</p>}
                <Button variant="success" className="w-100">Redeem Now</Button>
              </Card.Body>
            </Card>
          </Col>
        ))}
